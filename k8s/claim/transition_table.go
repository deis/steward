...
var (
	transitionTable = map[state.Current]nextFunc{
		...
		state.NewCurrentNoStatus(mode.ActionCreate, watch.Added): compoundNextFunc(processProvision, processBind),
		...
		state.NewCurrent(mode.StatusBound, mode.ActionDelete, watch.Modified): compoundNextFunc(processUnbind, processDeprovision),
